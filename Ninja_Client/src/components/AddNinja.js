...
class AddNinja extends React.Component {
  ...
  addNinja() {
    ...
    axios
      .post("http://localhost:8080/api/register/ninja", formData)
      .then(response => {
        this.setState({
  ...
  render() {
    return (
      <div style={{ width: 500, margin: "0px auto" }}>
        <h3 className="text-center pt-4 text-primary">Ninja</h3>
        <form>
        ...
        <div className="form-group">
          ...
          <span className="text-danger">
            {this.state.formErrors.ninjaNameErr}
          </span>
          ...
          <span className="text-danger">
            {this.state.formErrors.ninjaDesignationErr}
          </span>

          <button
            type="button"
            onClick={this.addNinja}
